...
;                                    (make-point 4 4)))
; (define rectangle 
;   (make-rectangle length-1 length-2 width-1 width-2))
;
...
(define (percent i)  
  (* 100 (abs (/ (width-interval i) (center i)))))  
...
;          ((not (pair? tree)) (append result (list tree)))
;          (else (append (if (null? (car tree)) (list (car tree))
;                            (fringe (car tree))) 
;                        (fringe (cdr tree))))))
;  (iter tree nil))
